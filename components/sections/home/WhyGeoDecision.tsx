      ...
      <div className="mx-auto max-w-[1700px] px-4 sm:px-6 lg:px-12 md:px-8 space-y-12">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {points.map((point, idx) => (
            ...
            >
              <div
                className="group relative h-full p-8 rounded-2xl border-2 border-slate-100 bg-white overflow-hidden transition-all duration-300 hover:border-[#16a34a]/30 hover:shadow-2xl hover:translate-y-[-4px] cursor-pointer"
              >
                {/* Background glow for cards - Red */}
                <div className="absolute -top-10 -right-10 h-32 w-32 bg-[#16a34a]/5 blur-3xl rounded-full group-hover:bg-[#16a34a]/10 transition-colors" />

                <div className="relative space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-[2px] bg-gradient-to-b from-[#16a34a] to-transparent" />
                    <h3 className="text-xl font-black text-slate-900 group-hover:text-[#16a34a] transition-colors">
                      {point.title}
                    </h3>
                  </div>
